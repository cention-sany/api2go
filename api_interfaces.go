...
// Use Responder for success status codes and content/meta data. In case of an
// error, use the error return value preferrably with an instance of our
// HTTPError struct.
type CRUD interface {
...
// The PaginatedFindAll interface can be optionally implemented to fetch a
// subset of all records. Pagination query parameters must be used to limit the
// result. Pagination URLs will automatically be generated by the api. You can
// use a combination of the following 2 query parameters:
...
// The FindAll interface can be optionally implemented to fetch all records at
// once.
...
// The ObjectInitializer interface can be implemented to have the ability to
// change a created object before Unmarshal is called. This is currently only
// called on Create as the other actions go through FindOne or FindAll which are
// already controlled by the implementer.
...
// The Responder interface is used by all Resource Methods as a container for
// the Response. Metadata is additional Metadata. You can put anything you like
// into it, see jsonapi spec. Result returns the actual payload. For FindOne,
// put only one entry in it. StatusCode sets the http status code.
type Responder interface {
	Result() interface{}
	StatusCode() int
}

// The LinksResponder interface may be used when the response object is able to
// return a set of links for the top-level response object.
